...
__webpack_require__(/*! ./bootstrap */ "./resources/js/bootstrap.js"); // Submit button to trigger search/scan


var submit = document.getElementById('search-submit'); // Container for result elements

var resultContainer = document.getElementById('result-container'); //Container for loading display elements

var loadingContainer = document.getElementById('loading-container'); // The collapseable DOM element

var resultContainerCollapseElement = document.getElementById('result-container-collapse'); // The bootstrap Collapse object

var resultContainerCollapse = new bootstrap.Collapse(resultContainerCollapseElement, {
  toggle: false
}); ///Fires when ANY collapse element from Bootstrap is fully collapses

resultContainerCollapseElement.addEventListener('hidden.bs.collapse', function () {
...
resultContainerCollapseElement.addEventListener('shown.bs.collapse', function () {
  fetch("/api/crawl/".concat(resultContainerCollapse.depth, "/").concat(resultContainerCollapse.url)).then(function (response) {
    return response.json();
  }).then(function (data) {
    // Check for an error being present instead of completed data.
    if (data.hasOwnProperty('status') && data.status == false) {
      document.getElementById('loading-container').innerHTML = "<h2 class='text-danger'>ERROR: ".concat(data.message, "</h2>");
    } else {
      ...
      document.getElementById('scan-title-length').innerHTML = Math.floor(data.map(function (item) {
        return item['pageTitle'].length;
      }).reduce(function (sum, item) {
        return sum += item;
      }) / data.length);
      document.getElementById('scan-table-body').innerHTML = data.map(function (item) {
        return "<tr><td>".concat(item['pageUrl'], "</td><td>").concat(statusToBadge(item['httpResponse']), "</td></tr>");
      }).join(' ');
    }
  });
}); //Convert the HTTP status into a coloured badge

statusToBadge = function statusToBadge(status) {
  statusType = Math.floor(parseInt(status) / 100);

  if (statusType == 2) {
    return "<span class=\"badge bg-success\">".concat(status, "</span>");
  } else if (statusType == 4) {
    return "<span class=\"badge bg-warning\">".concat(status, "</span>");
  } else if (statusType == 5) {
    return "<span class=\"badge bg-danger\">".concat(status, "</span>");
  } else {
    return "<span class=\"badge bg-secondary\">".concat(status, "</span>");
  }
}; // Function to handle kicking off a new search/crawl


loadCrawl = function loadCrawl(depth, url) {
...
averageResultSet = function averageResultSet(obj, property) {
  var count = 0;
  var iter = 0;
...
mergeResultSet = function mergeResultSet(obj, property) {
...
window.onload = function () {
  ...
  if (urlComponents.length > 4) {
    ...
    url = urlComponents.join('/'); // 2 argument are present in the URL
...
submit.addEventListener('click', function () {
  var url = document.getElementById('search-url');
  var depth = document.getElementById('search-url-depth');
  window.history.pushState("/".concat(depth.value, "/").concat(url.value), '', "/".concat(depth.value, "/").concat(url.value));
